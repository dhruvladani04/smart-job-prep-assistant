...
const ResumeRewriter = ({ existingRewrite = null }) => {
  ...
  const [activeTab, setActiveTab] = useState(existingRewrite?.bullets?.length > 0 ? 'upload' : 'manual'); // 'upload' or 'manual'

  // Initialize with existing bullets if available
  useEffect(() => {
    if (existingRewrite?.bullets?.length > 0) {
      setBullets(existingRewrite.bullets);
      if (existingRewrite.content) {
        setResumeBullet(existingRewrite.content);
      }
    }
  }, [existingRewrite]);
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {/* Bullet Selection (show when we have bullets from upload or existing rewrite) */}
        {(bullets.length > 0 || existingRewrite?.bullets?.length > 0) && activeTab === 'upload' && (
          <div className="space-y-2">
            <label htmlFor="resumeBullet" className="block text-sm font-medium text-gray-700">
              Select a bullet point to enhance
            </label>
            <select
              value={resumeBullet || (existingRewrite?.bullets?.[0] || '')}
              onChange={(e) => setResumeBullet(e.target.value)}
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
              {(bullets.length > 0 ? bullets : existingRewrite?.bullets || []).map((bullet, i) => (
